...
y = np.array([-1, 1, 1, -1]) # xor 연산w

w = np.array([1., 1., 1.]) # b, w1, w2

def feed_forward(param):
    return np.dot(param, w[1:]) + w[0]

def step_function(param): #activation function으로 사용 (활성함수)
    # 0보다 크면 1 아니면 -1을 리턴하는 계단 함수
