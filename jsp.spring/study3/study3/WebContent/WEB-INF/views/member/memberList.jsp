	...
	<div class="container">
		...
		<div class="panel panel-default collapse in" id="id_search_area">
			<div class="panel-body">
				<form name="search" action="memberList.wow" method="post"
								...
								<select id="id_searchJob" name="searchJob"
									...
									<c:forEach items="${jobList}" var="code">
										<option value="${code.commCd}"
											${searchVO.searchJob eq code.commCd ? "selected='selected'" : "" }>${code.commNm}</option>
									</c:forEach>
								</select>
							</div>
							...
							<div class="col-sm-2">
								<select id="id_search" name="searchHobby"
									...
									<c:forEach items="${hobbyList}" var="code">
										<option value="${code.commCd}"
											${searchVO.searchHobby eq code.commCd ? "selected='selected'" : "" }>${code.commNm}</option>
									</c:forEach>
								</select>
							</div>
						</div>
		...
		<div>
			<a href="memberForm.wow" class="btn btn-primary btn-sm pull-right">회원
				등록</a>
		</div>
			...
			<tbody>
				<c:forEach items="${MemberList }" var="MemberList">
					<tr>
						...
						<td><a href="memberView.wow?memId=${MemberList.memId }">
		...
		<nav class="text-center">
			<ul class="pagination">
				...
				<c:if test="${searchVO.curPage!=1 }">
					<li><a href="memberList.wow?curPage=${searchVO.curPage-1 }"
				...
				<!-- 마지막 페이지 -->
				<li><a
					href="memberList.wow?curPage=${searchVO.totalPageCount }"
